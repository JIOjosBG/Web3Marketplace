...
describe("SimpleSeller", async function () {
    let simpleSeller;
    let hashedData;
    const SimpleAuciotn = await ethers.getContractFactory("SimpleAuciotn");
    const accounts = await ethers.getSigners();

    describe("Deployment", async function () {
        beforeEach(async function ()  {
            simpleAuciotn = await SimpleAuciotn.connect(accounts[0]).deploy();
        });

        it("Should have the correct owner", async function () {
            const addressOfOwnerOfSimpleAuction = await simpleAuciotn.owner();
